...
/* *
// 1. read all jpegs, take exif data, use it to sort pictures
...
/* */
//2. read files, find date in filename, use it to sort pictures
fs.readdir(baseDir, function (error, files) {
  if (!error) {

    // Find images and read exif data
    for (var i = 0; i < files.length; i++) {
      if (files[i].match(/\.(mp4|jpg|jpeg)$/i)) {

        var filename = files[i];
        if (filename.match(/^[0-9]{8}_/)) {
          var date = filename.substr(0, 4) + '-' + filename.substr(4, 2) + '-' + filename.substr(6, 2);
          moveToDateDirectory(filename, date).then(
            function (data) {
              console.log('File "' + filename + '" moved to directory');
            },
            function (error) {
              console.log(error);
            }
